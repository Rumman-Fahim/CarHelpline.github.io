...
if (customFilter[0] != null) {
	for (let i = 0; i < customFilter.length; i++) {
		...
		let customFilter_arrowLeft = customFilter[i].querySelector(".navigation .arrow.left");
		let customFilter_arrowRight = customFilter[i].querySelector(".navigation .arrow.right");
		let customFilter_tabContainer = customFilter[i].querySelector(".navigation .tab-container");
		let customFilter_tabContainer_tabs = customFilter[i].querySelectorAll(".navigation .tab-container .tab");
		let customFilter_content = customFilter[i].querySelectorAll(".content-container .content");
	...
	function defaultFilterSettings(
			...
			scrollToActiveFilterOption(filterIndex, filterTabContainer, filterTabsArray);
			displayActiveFilterContent(filterIndex, filterTabsArray, filterContentArray);
	}
	...
	function scrollToActiveFilterOption(filterIndex, scrollabeTabContainer, tabsArray) {
	...
	function displayActiveFilterContent(filterIndex, tabsArray, contentArray) {
		let filterType = tabsArray[customFilterActiveIndex[filterIndex]].getAttribute("name");
...
let footerCards = document.querySelectorAll("#footerAccordian .card .card-body");
...
if (btnPrev != null) {
	...
	let carouselContainerWidth = document.querySelector(".multi-content-carousel")
		.offsetWidth;
	setCarContainerWidth();
	window.addEventListener("resize", () => {
		carouselContainerWidth = document.querySelector(".multi-content-carousel")
			.offsetWidth;
...
if (allCarousels[0] != null) {
	for (let i = 0; i < allCarousels.length; i++) {
		...
		let carousel_tabContainer = allCarousels[i].querySelector(".navigation .tabs-container");
		let carousel_arrow_left = allCarousels[i].querySelector(".navigation .arrow.left");
		let carousel_arrow_right = allCarousels[i].querySelector(".navigation .arrow.right");
		let carousel_tabContainer_tabs = allCarousels[i].querySelectorAll(".navigation .tabs-container .tab");
		let carousel_movableContentWrapper = allCarousels[i].querySelector(".content-container .movable-wrapper");
		let carousel_movableContentWrapper_content = allCarousels[i].querySelectorAll(".content-container .movable-wrapper .content");

		setFunctionForCarousel(i, carousel_tabContainer_tabs, carousel_arrow_left, carousel_arrow_right,
			carousel_tabContainer, carousel_movableContentWrapper, carousel_movableContentWrapper_content)
		
		defaultCarouselSettings(i, carousel_tabContainer_tabs, carousel_arrow_left, carousel_arrow_right,
		carousel_tabContainer, carousel_movableContentWrapper, carousel_movableContentWrapper_content)
	}
	
	function defaultCarouselSettings(carouselIndex, carousel_tabContainer_tabs, carousel_arrow_left, carousel_arrow_right, carousel_tabContainer,carousel_movableContentWrapper,carousel_movableContentWrapper_content) {
		scrollCarouselTabContainerToActiveTab(carouselIndex, carousel_tabContainer, carousel_tabContainer_tabs)
		sliderToActiveCarouselSlide(carouselIndex, carousel_movableContentWrapper, carousel_movableContentWrapper_content)
	}

	function setFunctionForCarousel(carouselIndex, carousel_tabContainer_tabs, carousel_arrow_left, carousel_arrow_right, carousel_tabContainer,carousel_movableContentWrapper,carousel_movableContentWrapper_content) {
		// Tabs
		...
		carousel_arrow_right.addEventListener("click", () => {
			if (customCarouselActiveIndex[carouselIndex] >= carousel_tabContainer_tabs.length - 1) {
				return;
			}
			customCarouselActiveIndex[carouselIndex]++;
			scrollCarouselTabContainerToActiveTab(carouselIndex, carousel_tabContainer, carousel_tabContainer_tabs)
	...
	function scrollCarouselTabContainerToActiveTab(carouselIndex, carousel_tabContainer, carousel_tabContainer_tabs) {
		carousel_tabContainer.scroll({
			left: carousel_tabContainer_tabs[customCarouselActiveIndex[carouselIndex]].offsetLeft,
			top: 0,
			behavior: "smooth",
		});
		for (let i = 0; i < carousel_tabContainer_tabs.length; i++) {
	...
	function sliderToActiveCarouselSlide(carouselIndex, carousel_movableContentWrapper, carousel_movableContentWrapper_content) {
		...
		carousel_movableContentWrapper.style.height = carousel_movableContentWrapper_content[customCarouselActiveIndex[carouselIndex]].innerHeight + 'px';
	}
}
